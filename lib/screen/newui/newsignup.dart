...
class _NewLoginState extends State<NewLogin> {
  ...
  void verifyNumber(BuildContext ctx, String mobile) {
    Provider.of<OTPProvider>(ctx, listen: false).sendOTP(mobile).then((value) {
      ...
      Scaffold.of(ctx).showSnackBar(snackbar).closed.then((value) {
        setState(() {
          showLoading = false;
          isOTPScreen = true;
        });
    ...
    Provider.of<OTPProvider>(context, listen: false)
      ...
      if (value.status == 200) {
        if (value.data.isNewUser == false) {
          ...
          Navigator.pushNamedAndRemoveUntil(ctx,NewHomepage.Tag,(route)=>false,arguments: {'id':0}).then((value) => dispose());
        } else {
          setState(() {
            isOTPScreen = false;
            isNewUser = true;
                                ...
                                child: TextFormField(
                                  ...
                                  decoration: InputDecoration(
                                      ...
                                      contentPadding:
                                          EdgeInsets.only(left: 15, top: 6),
                                      errorStyle: TextStyle(height: 0.5),
                                      hintText: 'e.g. example@youmail.com',
                                      filled: true,
                                      fillColor: Colors.white),
                                ),
                              )
        ...
        onWillPop: () {
          if (isNewUser) {
            ...
            Navigator.of(context)
                .pushReplacementNamed(NewHomepage.Tag, arguments: {'id': 0}).then((value) => dispose());
          }
          return null;
        });
  }
